...
namespace _703A
{
    class Program
    {
        ...
        {
            int testCases = int.Parse(Console.ReadLine()), Mishka = 0, Chris = 0;
            for(int i = 0; i< testCases; i++)
            {
                string[] dices = Console.ReadLine().Split();
                if (int.Parse(dices[0]) > int.Parse(dices[1]))
                {
                    Mishka++;
                }else if(int.Parse(dices[0]) < int.Parse(dices[1]))
                {
                    Chris++;
                }
            }
            switch (Mishka)
            {
                case var expression when (Mishka > Chris):
